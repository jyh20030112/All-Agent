...
class _StoreLockCancelled(Exception):
    pass
...
class StoreFileLock:
    """Process-local plus POSIX advisory lock for one Store sidecar file."""
    ...
    def acquire(
        ...
        if fcntl is None:
            raise SessionStoreError(
                "JsonlSessionStore cross-process locking requires POSIX"
            )
        ...
        try:
            ...
            try:
                ...
                raise SessionStoreError(
                    f"failed to open SessionStore lock {self.path}"
                ) from exc
        ...
        while True:
            ...
            try:
                ...
                if exc.errno not in {errno.EACCES, errno.EAGAIN}:
                    raise SessionStoreError(
                        f"failed to acquire SessionStore lock {self.path}"
                    ) from exc
            self._raise_if_timed_out(deadline)
    ...
    def _raise_if_timed_out(self, deadline: float | None) -> None:
        if deadline is not None and time.monotonic() >= deadline:
            raise SessionStoreLockTimeoutError(
                f"timed out acquiring SessionStore lock {self.path}"
            )

    @staticmethod
    def _raise_if_cancelled(cancelled: threading.Event | None) -> None:
        if cancelled is not None and cancelled.is_set():
            raise _StoreLockCancelled
