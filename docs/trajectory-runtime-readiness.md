...
The host remains responsible for the domain-specific evaluator. Core does not
...
| `completion_proposed` | after a text Completion Claim, before accepting terminal success | full Requirement/Constraint evaluation |
...
3. When an assistant message contains no Tool calls, capture
   ...
   but does not change the return value in this phase.
